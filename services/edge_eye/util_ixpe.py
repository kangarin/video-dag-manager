...
class MaterialDetection:
    def __init__(self, detection_area, buffer_size) -> None:
        ...
        self.background = raw_background[self.p1[1]
                                         :self.p2[1], self.p1[0]:self.p2[0]]
        # 记得初始化这个玩意
        self.counter = 0
...
class CalPosition:
    ...
    def calculatePosInMROI(self, mroi, position='left', abs_point=(0, 0)):
        ...
        common_points = self.commom_points[relative_py:relative_py +
                                                       width, relative_px:relative_px + height]  # xxx
